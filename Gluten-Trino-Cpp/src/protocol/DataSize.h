...
class DataSize {
 public:
  ...
  DataSize(double value, DataUnit dataUnit) : value_(value), dataUnit_(dataUnit) {}

  double getValue() const { return value_; }

  DataUnit getDataUnit() const { return dataUnit_; }

  std::string toString() const;

  double getValue(DataUnit dataUnit) const {
    return value_ * (toBytesPerDataUnit(this->dataUnit_) / toBytesPerDataUnit(dataUnit));
  }

  DataSize operator+=(const DataSize& other) {
    assert(this->getDataUnit() == other.getDataUnit());
