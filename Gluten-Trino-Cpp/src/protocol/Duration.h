...
class DurationTimeUnitUnsupported : public std::invalid_argument {
 public:
  DurationTimeUnitUnsupported() : std::invalid_argument("Unsupported time unit") {}
};
...
class Duration {
 public:
  ...
  Duration(double value, TimeUnit timeUnit) : value_(value), timeUnit_(timeUnit) {}
  ...
  double getValue(TimeUnit timeUnit) const {
    return value_ * (toMillisPerTimeUnit(timeUnit_) / toMillisPerTimeUnit(timeUnit));
  }

  Duration operator+=(const Duration& other) {
    assert(this->getTimeUnit() == other.getTimeUnit());
