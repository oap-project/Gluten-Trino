...
class ConfigBase {
 public:
  ...
  void initialize(std::unique_ptr<facebook::velox::Config>&& config) {
  ...
  folly::Optional<std::string> setValue(const std::string& propertyName,
  ...
  T requiredProperty(const std::string& propertyName) const {
    ...
    if (propertyValue.has_value()) {
      ...
      VELOX_USER_FAIL("{} is required in the {} file.", propertyName, filePath_);
    }
  }
  ...
  folly::Optional<std::string> optionalProperty(const std::string& propertyName) const {
    auto val = config_->get(propertyName);
    if (!val.hasValue()) {
      const auto it = registeredProps_.find(propertyName);
      if (it != registeredProps_.end()) {
  ...
  /// Returns optional value of the string type. Can return folly::none.
  folly::Optional<std::string> optionalProperty(std::string_view propertyName) const {
 ...
 protected:
  ...
  std::unique_ptr<facebook::velox::Config> config_;
  std::string filePath_;
  // Map of registered properties with their default values.
  std::unordered_map<std::string, folly::Optional<std::string>> registeredProps_;
};
...
class SystemConfig : public ConfigBase {
 public:
  ...
  static constexpr std::string_view kHttpServerHttpPort{"http-server.http.port"};
  ...
  /// startup.
  static constexpr std::string_view kHttpServerReusePort{"http-server.reuse-port"};
  static constexpr std::string_view kDiscoveryUri{"discovery.uri"};
  static constexpr std::string_view kMaxDriversPerTask{"task.max-drivers-per-task"};
  ...
  static constexpr std::string_view kHttpServerHttpsPort{"http-server.https.port"};
  static constexpr std::string_view kHttpServerHttpsEnabled{"http-server.https.enabled"};
  static constexpr std::string_view kHttpsSupportedCiphers{"https-supported-ciphers"};
  ...
  static constexpr std::string_view kNumConnectorIoThreads{"num-connector-io-threads"};
  ...
  static constexpr std::string_view kSpillerSpillPath{"experimental.spiller-spill-path"};
  ...
  static constexpr std::string_view kAsyncDataCacheEnabled{"async-data-cache-enabled"};
  ...
  static constexpr std::string_view kMmapArenaCapacityRatio{"mmap-arena-capacity-ratio"};
  static constexpr std::string_view kUseMmapAllocator{"use-mmap-allocator"};
  static constexpr std::string_view kEnableVeloxTaskLogging{"enable_velox_task_logging"};
  ...
  static constexpr std::string_view kHttpEnableAccessLog{"http-server.enable-access-log"};
  static constexpr std::string_view kHttpEnableStatsFilter{
      "http-server.enable-stats-filter"};
  static constexpr std::string_view kRegisterTestFunctions{"register-test-functions"};
  ...
  static constexpr std::string_view kQueryMaxMemoryPerNode{"query.max-memory-per-node"};
  ...
  static constexpr std::string_view kEnableMemoryLeakCheck{"enable-memory-leak-check"};
...
class NodeConfig : public ConfigBase {
 public:
  ...
  std::string nodeIp(const std::function<std::string()>& defaultIp = nullptr) const;
...
/// facebook::velox::QueryConfig. Not designed for actual property access during a query
/// run.
