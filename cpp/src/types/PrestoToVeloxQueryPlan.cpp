...
connector::hive::HiveColumnHandle::ColumnType toHiveColumnType(
  ...
  switch (type) {
    ...
    default:
      VELOX_UNSUPPORTED("Unsupported Hive column type: {}.", toJsonString(type));
  }
}
...
std::shared_ptr<connector::ColumnHandle> toColumnHandle(
  ...
  if (auto hiveColumn = dynamic_cast<const protocol::HiveColumnHandle*>(column)) {
    return std::make_shared<connector::hive::HiveColumnHandle>(
        hiveColumn->name, toHiveColumnType(hiveColumn->columnType),
...
std::shared_ptr<connector::ConnectorTableHandle> toConnectorTableHandle(
    ...
    std::unordered_map<std::string, std::shared_ptr<connector::ColumnHandle>>&
        partitionColumns) {
  if (auto hiveTable = std::dynamic_pointer_cast<const protocol::HiveTableHandle>(
          tableHandle.connectorHandle)) {
    // Use fully qualified name if available.
    std::string tableName =
        hiveTable->schemaName.empty()
            ? hiveTable->tableName
...
bool isFixedPartition(const std::shared_ptr<const protocol::ExchangeNode>& node,
                      protocol::SystemPartitionFunction partitionFunction) {
  if (node->type != protocol::ExchangeNodeType::REPARTITION) {
    return false;
  }

  auto connectorHandle = node->partitioningScheme.partitioning.handle.connectorHandle;
  auto handle =
      std::dynamic_pointer_cast<protocol::SystemPartitioningHandle>(connectorHandle);
  if (!handle) {
    return false;
  }
...
bool isHashPartition(const std::shared_ptr<const protocol::ExchangeNode>& node) {
  return isFixedPartition(node, protocol::SystemPartitionFunction::HASH);
}

bool isRoundRobinPartition(const std::shared_ptr<const protocol::ExchangeNode>& node) {
...
core::LocalPartitionNode::Type toLocalExchangeType(protocol::ExchangeNodeType type) {
  switch (type) {
...
core::PlanNodePtr VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  VELOX_USER_CHECK(node->scope == protocol::ExchangeNodeScope::LOCAL,
  ...
  if (node->orderingScheme) {
    ...
    return std::make_shared<core::LocalMergeNode>(node->id, sortingKeys, sortingOrders,
  ...
  for (auto i = 0; i < sourceNodes.size(); ++i) {
    ...
    sourceNodes[i] = std::make_shared<core::ProjectNode>(
        fmt::format("{}.{}", node->id, i), std::move(names), std::move(projections),
        sourceNodes[i]);
  }

  if (isHashPartition(node)) {
    auto partitionKeys =
        toFieldExprs(node->partitioningScheme.partitioning.arguments, exprConverter_);
  ...
  if (isRoundRobinPartition(node)) {
    return std::make_shared<core::LocalPartitionNode>(
        node->id, type, std::make_shared<RoundRobinPartitionFunctionSpec>(),
        std::move(sourceNodes));
  }
  ...
  VELOX_UNSUPPORTED("Unsupported flavor of local exchange: {}", toJsonString(node));
}
...
std::shared_ptr<const core::ValuesNode> VeloxQueryPlanConverterBase::toVeloxQueryPlan(
    const std::shared_ptr<const protocol::ValuesNode>& node,
    const std::shared_ptr<protocol::TableWriteInfo>&, const protocol::TaskId& taskId) {
  ...
  auto rowVector = std::make_shared<RowVector>(pool_, rowType, BufferPtr(), numRows,
                                               std::move(vectors), 0);
  ...
  return std::make_shared<core::ValuesNode>(node->id,
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  for (const auto& aggregation : node->aggregations) {
    outputVariables.push_back(aggregation.first);
  }
  toAggregations(outputVariables, node->aggregations, aggregates, aggregateNames);
...
void VeloxQueryPlanConverterBase::toAggregations(
  ...
  for (const auto& entry : outputVariables) {
    ...
    if (prestoAggregation.orderingScheme != nullptr) {
      for (const auto& orderBy : prestoAggregation.orderingScheme->orderBy) {
        aggregate.sortingKeys.emplace_back(exprConverter_.toVeloxExpr(orderBy.variable));
        aggregate.sortingOrders.emplace_back(toVeloxSortOrder(orderBy.sortOrder));
      }
    }
...
core::PlanNodePtr VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  if (node->criteria.empty() && core::isInnerJoin(joinType) && !node->filter) {
    return std::make_shared<core::NestedLoopJoinNode>(
        node->id, toVeloxQueryPlan(node->left, tableWriteInfo, taskId),
  ...
  return std::make_shared<core::HashJoinNode>(
      node->id, joinType, false, leftKeys, rightKeys,
...
core::PlanNodePtr VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  return std::make_shared<core::MarkDistinctNode>(
      node->id, node->markerVariable.name, toVeloxExprs(node->distinctVariables),
      toVeloxQueryPlan(node->source, tableWriteInfo, taskId));
}
...
std::shared_ptr<const core::TopNNode> VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  return std::make_shared<core::TopNNode>(
      node->id, sortingKeys, sortingOrders, node->count,
      node->step == protocol::Step::PARTIAL,
      toVeloxQueryPlan(node->source, tableWriteInfo, taskId));
}

std::shared_ptr<const core::LimitNode> VeloxQueryPlanConverterBase::toVeloxQueryPlan(
    const std::shared_ptr<const protocol::LimitNode>& node,
    const std::shared_ptr<protocol::TableWriteInfo>& tableWriteInfo,
    const protocol::TaskId& taskId) {
  return std::make_shared<core::LimitNode>(
      node->id, 0, node->count, node->step == protocol::LimitNodeStep::PARTIAL,
      toVeloxQueryPlan(node->source, tableWriteInfo, taskId));
}

std::shared_ptr<const core::OrderByNode> VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  return std::make_shared<core::OrderByNode>(
      node->id, sortingKeys, sortingOrders, node->isPartial,
      toVeloxQueryPlan(node->source, tableWriteInfo, taskId));
}
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
    ...
    const std::shared_ptr<protocol::TableWriteInfo>& tableWriteInfo,
    const protocol::TaskId& taskId) {
  auto prestoTaskId = TrinoTaskId(taskId);
  ...
  return std::make_shared<core::AssignUniqueIdNode>(
      node->id, node->idVariable.name, taskUniqueId,
      toVeloxQueryPlan(node->source, tableWriteInfo, taskId));
}
...
core::PlanNodePtr VeloxQueryPlanConverterBase::toVeloxQueryPlan(
    ...
    const std::shared_ptr<protocol::TableWriteInfo>& tableWriteInfo,
    const protocol::TaskId& taskId) {
  if (auto exchange = std::dynamic_pointer_cast<const protocol::ExchangeNode>(node)) {
  ...
  if (auto values = std::dynamic_pointer_cast<const protocol::ValuesNode>(node)) {
 ...
 */
  ...
  if (auto join = std::dynamic_pointer_cast<const protocol::SemiJoinNode>(node)) {
    return toVeloxQueryPlan(join, tableWriteInfo, taskId);
  }
  /*
  if (auto join =
...
core::PlanFragment VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  */
  ...
  if (auto output =
          std::dynamic_pointer_cast<const protocol::OutputNode>(fragment.root)) {
  ...
  for (const auto& expr : partitioningKeys) {
    ...
    if (channel == kConstantChannel) {
      ...
      VELOX_CHECK(expr, "Not a ConstantTypedExpr, cannot convert.");
  ...
  if (auto systemPartitioningHandle =
    ...
    planFragment.planNode = std::make_shared<core::PartitionedOutputNode>(
        "root", core::PartitionedOutputNode::Kind::kPartitioned, partitioningKeys,
        numPartitions, partitioningScheme.replicateNullsAndAny,
        std::make_shared<HivePartitionFunctionSpec>(hivePartitioningHandle->bucketCount,
                                                    bucketToPartition, keyChannels,
                                                    constValues),
        toRowType(partitioningScheme.outputLayout), sourceNode);
    return planFragment;
  } else {
...
core::PlanNodePtr VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  return core::PartitionedOutputNode::single(
      node->id, toRowType(node->outputVariables),
      VeloxQueryPlanConverterBase::toVeloxQueryPlan(node->source, tableWriteInfo,
                                                    taskId));
}
...
core::PlanNodePtr VeloxInteractiveQueryPlanConverter::toVeloxQueryPlan(
  ...
  if (node->orderingScheme) {
    ...
    return std::make_shared<core::MergeExchangeNode>(node->id, rowType, sortingKeys,
                                                     sortingOrders);
  }
