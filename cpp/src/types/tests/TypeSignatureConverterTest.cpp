...
TypePtr signature(std::string typeName) { return typeFromString(typeName); }

NamedType namedParameter(std::string name, TypePtr type) { return NamedType{name, type}; }

NamedType namedParameter(std::string name, bool _unused, TypePtr type) {
  return namedParameter(name, type);
}

NamedType unnamedParameter(TypePtr type) { return NamedType{"", type}; }

TypePtr varchar() { return typeFromString("VARCHAR"); }

TypePtr varchar(int size) { return typeFromString("VARCHAR"); }

TypePtr array(TypePtr type) { return arrayFromType(type); }
TypePtr map(TypePtr key, TypePtr value) { return mapFromKeyValueType(key, value); }
...
#define assertEquals(input, expected) ASSERT_EQ(input->toString(), expected->toString())
...
TEST_F(TestTypeSignature, sig01) { assertSignature("boolean", "BOOLEAN"); }

TEST_F(TestTypeSignature, sig02) { assertSignature("varchar", "VARCHAR"); }

TEST_F(TestTypeSignature, sig03) {
  assertEquals(parseTypeSignature("int"), parseTypeSignature("integer"));
}

TEST_F(TestTypeSignature, sig04) { assertSignature("array(bigint)", "ARRAY<BIGINT>"); }
...
TEST_F(TestTypeSignature, sig07) {
  assertEquals(parseTypeSignature("array(array(int))"),
...
TEST_F(TestTypeSignature, sig10) {
  assertSignature("map(bigint,map(bigint,map(varchar,bigint)))",
                  "MAP<BIGINT,MAP<BIGINT,MAP<VARCHAR,BIGINT>>>");
}

TEST_F(TestTypeSignature, sig11) { assertSignatureFail("blah()"); }

TEST_F(TestTypeSignature, sig12) { assertSignatureFail("array()"); }

TEST_F(TestTypeSignature, sig13) { assertSignatureFail("map()"); }

TEST_F(TestTypeSignature, sig14) { assertSignatureFail("x"); }
...
TEST_F(TestTypeSignature, row07) {
  assertRowSignature(
      ...
      array(rowSignature(namedParameter("col0", false, signature("bigint")),
...
TEST_F(TestTypeSignature, row11) {
  assertRowSignature(
      ...
      rowSignature(unnamedParameter(varchar(10)),
                   namedParameter("b", false,
                                  rowSignature(unnamedParameter(signature("bigint"))))));
}
...
TEST_F(TestTypeSignature, row13) {
  assertRowSignature("row(col0 array(row(bigint,double)))",
                     rowSignature(namedParameter(
                         "col0", false,
                         array(rowSignature(unnamedParameter(signature("bigint")),
                                            unnamedParameter(signature("double")))))));
}
...
TEST_F(TestTypeSignature, row15) {
  assertRowSignature("row(double precision)",
...
TEST_F(TestTypeSignature, row17) {
  ...
  assertEquals(parseTypeSignature("row(col iNt)"),
...
TEST_F(TestTypeSignature, spaces04) {
  assertRowSignatureContainsThrows(
      ...
      rowSignature(
          namedParameter("interval", false, signature("interval year to month"))),
      VeloxUserError, "Specified element is not found : INTERVAL YEAR TO MONTH");
}
...
TEST_F(TestTypeSignature, spaces06) {
  ...
  assertRowSignatureContainsThrows(
      ...
      rowSignature(unnamedParameter(signature("time with time zone"))), VeloxUserError,
...
TEST_F(TestTypeSignature, spaces09) {
  assertRowSignatureContainsThrows(
      ...
      rowSignature(unnamedParameter(signature("interval year to month"))), VeloxUserError,
...
TEST_F(TestTypeSignature, functionType) {
  ASSERT_THROWS_CONTAINS_MESSAGE(
      ...
      , VeloxUserError, "Failed to parse type [function(boolean,varchar(5),boolean)]");
}

TEST_F(TestTypeSignature, decimalType) {
  assertSignature("decimal(10, 5)", "DECIMAL(10,5)");
  assertSignature("decimal(20,10)", "DECIMAL(20,10)");
  ASSERT_THROWS_CONTAINS_MESSAGE(parseTypeSignature("decimal");
                                 , VeloxUserError, "Failed to parse type [decimal]");
  ASSERT_THROWS_CONTAINS_MESSAGE(parseTypeSignature("decimal()");
                                 , VeloxUserError, "Failed to parse type [decimal()]");
  ASSERT_THROWS_CONTAINS_MESSAGE(parseTypeSignature("decimal(20)");
