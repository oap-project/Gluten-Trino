...
std::string mapScalarFunction(const std::string& name) {
  static const std::unordered_map<std::string, std::string> kFunctionNames = {
      ...
      {"presto.default.$operator$is_distinct_from", "presto.default.distinct_from"},
...
std::string getFunctionName(const protocol::SqlFunctionId& functionId) {
  ...
  return nameEnd != std::string::npos ? functionId.substr(0, nameEnd) : functionId;
}

}  // namespace

velox::variant VeloxExprConverter::getConstantValue(const velox::TypePtr& type,
  ...
  switch (typeKind) {
    case TypeKind::HUGEINT:
      return valueVector->as<velox::SimpleVector<velox::int128_t>>()->valueAt(0);
    ...
    case TypeKind::TIMESTAMP:
      return valueVector->as<velox::SimpleVector<velox::Timestamp>>()->valueAt(0);
    ...
    case TypeKind::VARCHAR:
      return velox::variant(
          valueVector->as<velox::SimpleVector<velox::StringView>>()->valueAt(0));
    case TypeKind::VARBINARY:
      return velox::variant::binary(
          valueVector->as<velox::SimpleVector<velox::StringView>>()->valueAt(0));
    default:
      throw std::invalid_argument("Unexpected Block type: " +
...
std::optional<TypedExprPtr> tryConvertCast(const protocol::Signature& signature,
...
std::optional<TypedExprPtr> tryConvertTry(const protocol::Signature& signature,
                                          const std::string& returnType,
                                          const std::vector<TypedExprPtr>& args) {
...
std::optional<TypedExprPtr> VeloxExprConverter::tryConvertDate(
  ...
  auto builtin =
      std::static_pointer_cast<protocol::BuiltInFunctionHandle>(pexpr.functionHandle);
...
std::optional<TypedExprPtr> VeloxExprConverter::tryConvertLike(
  ...
  auto likePatternExpr =
      std::dynamic_pointer_cast<const protocol::CallExpression>(pexpr.arguments[1]);
  VELOX_CHECK_NOT_NULL(likePatternExpr);
  auto likePatternBuiltin = std::static_pointer_cast<protocol::BuiltInFunctionHandle>(
  ...
  if (likePatternSignature.name == kCast) {
    ...
    VELOX_FAIL("Unexpected like signature: {}", toJsonString(pexpr.arguments[1]));
  }
...
TypedExprPtr VeloxExprConverter::toVeloxExpr(
  ...
  if (auto builtin = std::dynamic_pointer_cast<protocol::BuiltInFunctionHandle>(
    ...
    auto returnType = parseTypeSignature(pexpr.returnType);
    if(getFunctionName(signature) == "presto.default.avg" || getFunctionName(signature) == "presto.default.sum") {
      if(args[0]->type()->isDecimal()) {
        auto varbinary = facebook::velox::VarbinaryType::create();
        return std::make_shared<CallTypedExpr>(varbinary, args, getFunctionName(signature));
      }
    }
    return std::make_shared<CallTypedExpr>(returnType, args, getFunctionName(signature));
  } /* else if (
...
std::shared_ptr<const ConstantTypedExpr> VeloxExprConverter::toVeloxExpr(
  ...
  switch (type->kind()) {
    ...
    case TypeKind::MAP: {
      ...
      return std::make_shared<ConstantTypedExpr>(
          std::make_shared<velox::ConstantVector<velox::ComplexType>>(pool_, 1, 0,
                                                                      valueVector));
    }
    default: {
...
std::shared_ptr<const CallTypedExpr> makeEqualsExpr(const TypedExprPtr& a,
                                                    const TypedExprPtr& b) {
  std::vector<TypedExprPtr> inputs{a, b};
  return std::make_shared<CallTypedExpr>(velox::BOOLEAN(), std::move(inputs),
                                         "presto.default.eq");
}

std::shared_ptr<const CastTypedExpr> makeCastExpr(const TypedExprPtr& expr,
...
std::shared_ptr<const CallTypedExpr> convertSwitchExpr(const velox::TypePtr& returnType,
      ...
      if (call->name() == "when") {
        ...
        if (valueIsTrue) {
          ...
          if (condition->type()->kindEquals(valueExpr->type())) {
            ...
            inputs.emplace_back(
                makeEqualsExpr(makeCastExpr(condition, valueExpr->type()), valueExpr));
          }
        }
  ...
  return std::make_shared<CallTypedExpr>(returnType, std::move(inputs), "switch");
}

TypedExprPtr convertBindExpr(const std::vector<TypedExprPtr>& args) {
  VELOX_CHECK_GE(args.size(), 2, "BIND expression must have at least two arguments");
  ...
  for (auto i = 0; i < args.size() - 1; i++) {
    const auto& field = std::dynamic_pointer_cast<const FieldAccessTypedExpr>(args[i]);
    mapping.insert({signature->nameOf(i), field});
  }
  ...
  return std::make_shared<LambdaTypedExpr>(newSignature,
                                           lambda->body()->rewriteInputNames(mapping));
}
...
TypedExprPtr convertDereferenceExpr(const velox::TypePtr& returnType,
                                    const std::vector<TypedExprPtr>& args) {
...
TypedExprPtr convertNullIfExpr(const velox::TypePtr& returnType,
                               const std::vector<TypedExprPtr>& args) {
  ...
  std::vector<TypedExprPtr> newArgs = {
      std::make_shared<CallTypedExpr>(velox::BOOLEAN(), args, "presto.default.eq"),
      std::make_shared<ConstantTypedExpr>(returnType,
                                          velox::variant::null(returnType->kind())),
      args[0]};
...
TypedExprPtr VeloxExprConverter::toVeloxExpr(
  ...
  if (pexpr->form == protocol::Form::ROW_CONSTRUCTOR) {
    return std::make_shared<CallTypedExpr>(returnType, std::move(args),
                                           "row_constructor");
  }
  ...
  return std::make_shared<CallTypedExpr>(returnType, args, mapScalarFunction(form));
}
...
std::shared_ptr<const FieldAccessTypedExpr> VeloxExprConverter::toVeloxExpr(
  ...
  return std::make_shared<FieldAccessTypedExpr>(parseTypeSignature(pexpr->type),
                                                pexpr->name);
}
...
TypedExprPtr VeloxExprConverter::toVeloxExpr(
  ...
  if (auto constant = std::dynamic_pointer_cast<protocol::ConstantExpression>(pexpr)) {
    return toVeloxExpr(constant);
  }
  if (auto special = std::dynamic_pointer_cast<protocol::SpecialFormExpression>(pexpr)) {
    return toVeloxExpr(special);
  }
  if (auto variable =
          std::dynamic_pointer_cast<protocol::VariableReferenceExpression>(pexpr)) {
  ...
  */
  ...
  throw std::invalid_argument("Unsupported RowExpression type: " + pexpr->_type);
}
