...
class TrinoExchangeSource : public facebook::velox::exec::ExchangeSource {
 public:
  class RetryState {
   public:
    explicit RetryState(int64_t maxWaitMs = 1'000)
    ...
    bool isExhausted() const {
      return facebook::velox::getCurrentTimeMs() - startMs_ > maxWaitMs_;
    }
  ...
  folly::SemiFuture<Response> request(uint32_t maxBytes,
  ...
  static std::unique_ptr<ExchangeSource> create(
      ...
      std::shared_ptr<facebook::velox::exec::ExchangeQueue> queue,
      facebook::velox::memory::MemoryPool* pool,
      folly::CPUThreadPoolExecutor* cpuExecutor, folly::IOThreadPoolExecutor* ioExecutor);
 ...
 private:
  ...
  void processDataError(const std::string& path, uint32_t maxBytes,
                        uint32_t maxWaitSeconds, const std::string& error,
                        bool retry = true);
