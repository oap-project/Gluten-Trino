        ...
        implements TaskExecution
{
    ...
    protected final TaskStateMachine taskStateMachine;
    protected final TaskContext taskContext;
    protected final OutputBuffer outputBuffer;

    protected final TaskHandle taskHandle;
    protected final TaskExecutor taskExecutor;

    protected final Executor notificationExecutor;

    protected final SplitMonitor splitMonitor;
    protected final DriverAndTaskTerminationTracker driverAndTaskTerminationTracker;

    protected final Map<PlanNodeId, DriverSplitRunnerFactory> driverRunnerFactoriesWithSplitLifeCycle;
    protected final List<DriverSplitRunnerFactory> driverRunnerFactoriesWithTaskLifeCycle;
    protected final Map<PlanNodeId, DriverSplitRunnerFactory> driverRunnerFactoriesWithRemoteSource;
    protected final List<DriverSplitRunnerFactory> allDriverRunnerFactories;

    @GuardedBy("this")
    protected final Map<PlanNodeId, Long> maxAcknowledgedSplitByPlanNode = new HashMap<>();

    @GuardedBy("this")
    protected final List<PlanNodeId> sourceStartOrder;
    @GuardedBy("this")
    protected int schedulingPlanNodeOrdinal;

    @GuardedBy("this")
    protected final Map<PlanNodeId, PendingSplitsForPlanNode> pendingSplitsByPlanNode;

    // number of created PrioritizedSplitRunners that haven't yet finished
    protected final AtomicLong remainingSplitRunners = new AtomicLong();
