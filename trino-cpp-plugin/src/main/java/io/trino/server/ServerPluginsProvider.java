        ...
        implements PluginsProvider
{
    ...
    private final File trinoCppPluginsDir;
    ...
    public ServerPluginsProvider(ServerPluginsProviderConfig config, @ForStartup Executor executor)
    {
        ...
        this.trinoCppPluginsDir = config.getTrinoCppPluginsDir();
    ...
    {
        List<URL> trinoCppClassPath = buildCppPluginClassPath(trinoCppPluginsDir);
        executeUntilFailure(
                executor,
                listFiles(installedPluginsDir).stream()
                        ...
                        .map(file -> (Callable<?>) () -> {
                            loader.load(file.getAbsolutePath(), () ->
                                    createClassLoader.create(file.getName(), combineList(trinoCppClassPath, buildClassPath(file))));
                            return null;
                        })
                        .collect(toImmutableList()));
    }

    private static List<URL> combineList(List<URL> left, List<URL> right)
    {
        return Stream.concat(left.stream(), right.stream()).toList();
    }

    private static List<URL> buildCppPluginClassPath(File path)
    {
        List<URL> list = new ArrayList<>();
        if (path != null) {
            list.add(fileToUrl(path));
        }
        return list;
    }
