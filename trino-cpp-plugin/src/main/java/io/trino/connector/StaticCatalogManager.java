        ...
        implements CatalogManager, ConnectorServicesProvider
{
    ...
    private final JsonCodec<MockCatalogProperties> catalogPropertiesJsonCodec;
    ...
    public StaticCatalogManager(CatalogFactory catalogFactory, StaticCatalogManagerConfig config, @ForStartup Executor executor)
    {
        ...
        this.catalogPropertiesJsonCodec = jsonCodec(MockCatalogProperties.class);
    }
                ...
                catalogProperties.stream()
                        .map(catalog -> (Callable<?>) () -> {
                            ...
                            if (WHITE_LIST.contains(connectorName)) {
                                MockCatalogProperties mockCatalogProperties = new MockCatalogProperties(catalogName, connectorName, catalog.getProperties());
                                registerConnector(catalogPropertiesJsonCodec.toJson(mockCatalogProperties));
                            }
                            return null;
                        })
                        .collect(toImmutableList()));
