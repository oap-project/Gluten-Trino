...
class GlutenSubfieldTokenizer
        implements Iterator<GlutenSubfield.PathElement>
{
    ...
    private GlutenSubfield.PathElement next;

    public GlutenSubfieldTokenizer(String path)
    {
    ...
    @Override
    public final GlutenSubfield.PathElement next()
    {
        if (!hasNext()) {
            throw new NoSuchElementException();
        }
        state = State.NOT_READY;
        GlutenSubfield.PathElement result = next;
        next = null;
        return result;
    }
    ...
    private GlutenSubfield.PathElement computeNext()
    {
        if (!hasNextCharacter()) {
            state = State.DONE;
            return null;
        ...
        if (tryMatch(DOT)) {
            GlutenSubfield.PathElement token = matchPathSegment();
            firstSegment = false;
            return token;
        }

        if (tryMatch(OPEN_BRACKET)) {
            GlutenSubfield.PathElement token = tryMatch(QUOTE) ? matchQuotedSubscript() : tryMatch(WILDCARD) ? matchWildcardSubscript() : matchUnquotedSubscript();
    ...
    private GlutenSubfield.PathElement matchPathSegment()
    {
        ...
        return new GlutenSubfield.NestedField(token);
    }

    private GlutenSubfield.PathElement matchWildcardSubscript()
    {
        return GlutenSubfield.allSubscripts();
    }

    private GlutenSubfield.PathElement matchUnquotedSubscript()
    {
        // seek until we see a special character or whitespace
        int start = index;
        ...
        return new GlutenSubfield.LongSubscript(index);
    }

    private GlutenSubfield.PathElement matchQuotedSubscript()
    {
        ...
        return new GlutenSubfield.StringSubscript(index);
    }
