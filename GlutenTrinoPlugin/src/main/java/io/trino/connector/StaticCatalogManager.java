        ...
        implements CatalogManager, ConnectorServicesProvider
{
    ...
    private static final List<String> WHITE_LIST = Arrays.asList("hive", "tpch");
    ...
    private final JsonCodec<GlutenCatalogProperties> catalogPropertiesJsonCodec;
    ...
    public StaticCatalogManager(CatalogFactory catalogFactory, StaticCatalogManagerConfig config, @ForStartup Executor executor)
    {
        ...
        this.catalogPropertiesJsonCodec = jsonCodec(GlutenCatalogProperties.class);
    }
        ...
        executeUntilFailure(
                ...
                catalogProperties.stream()
                        .map(catalog -> (Callable<?>) () -> {
                            ...
                            String connectorName = catalog.getConnectorName().toString();
                            log.info("-- Added catalog %s using connector %s --", catalogName, connectorName);
                            // Register Connector in native worker
                            if (WHITE_LIST.contains(connectorName)) {
                                GlutenCatalogProperties glutenCatalogProperties = new GlutenCatalogProperties(catalogName, connectorName, catalog.getProperties());
                                registerConnector(catalogPropertiesJsonCodec.toJson(glutenCatalogProperties));
                            }
                            return null;
                        })
                        .collect(toImmutableList()));
