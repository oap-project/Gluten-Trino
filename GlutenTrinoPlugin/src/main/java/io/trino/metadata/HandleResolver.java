...
public final class HandleResolver
{
    ...
    @SuppressWarnings("MethodMayBeStatic")
    public String getGlutenId(Object tableHandle)
    {
        Class<?> handleClass = tableHandle.getClass();
        //String _classLoaderId = classLoaderId(handleClass);
        String classLoaderId = "system";

        String className = handleClass.getSimpleName();
        if (className.startsWith("Gluten")) {
            className = className.substring("Gluten".length());
        }

        String packageName = handleClass.getPackageName();
        return classLoaderId + ":" + packageName + "." + className;
    }
