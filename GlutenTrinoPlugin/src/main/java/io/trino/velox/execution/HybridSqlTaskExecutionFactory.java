        ...
        extends SqlTaskExecutionFactory
{
    ...
    {
        GlutenPlanFragment newFragment = translatePlanFragment(fragment, session);
        if (newFragment != null) {
            try {
    ...
    private TaskExecution createNativeSqlTaskExecution(Session session, QueryContext queryContext, TaskStateMachine taskStateMachine, OutputBuffer outputBuffer, GlutenPlanFragment newFragment, Runnable notifyStatusChanged)
    {
    ...
    private GlutenPlanFragment translatePlanFragment(PlanFragment fragment, Session session)
    {
        logger.debug("Translating plan fragment %s.", fragment.getId().toString());
        // Try to convert PlanFragment to GlutenPlanNodes.
        try {
